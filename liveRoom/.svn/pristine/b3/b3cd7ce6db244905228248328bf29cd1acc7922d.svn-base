...
public class ChangeImageSize {
     ...
     * @param flag         缩放选择:true 放大; false 缩小;
     */
    public static void scaleBYWH(String srcImageFile, String result, int num) {
        try {
            ...
            int count = 1;
            int sht = width > height ? height : width;
            ...
            Image image = src.getScaledInstance((width/(count/2)), (height/(count/2)), Image.SCALE_SMOOTH);
            BufferedImage tag = new BufferedImage((width/(count/2)), (height/(count/2)), BufferedImage.TYPE_INT_RGB);
            Graphics g = tag.getGraphics();
            g.drawImage(image, 0, 0, null); // 绘制缩小后的图
            g.dispose();
            ImageIO.write(tag, "JPEG", new File(result));// 输出到文件流
